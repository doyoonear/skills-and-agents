...
- 코드와 함께 바뀌어야 하는 문서는 프로젝트 repo에 둔다.
- 프로젝트 하나를 넘어 재사용되는 지식은 Obsidian에 둔다.
- 둘 다 필요한 경우에도 역할을 분리한다.
  - repo docs: 실행 가능한 정책, ADR 정본, runbook, 코드 계약
  - Obsidian: 상위 지식, 요약, 링크, 일반화된 원칙
...
| 기준 | 예시 |
|---|---|
| 코드 변경과 함께 PR에서 리뷰되어야 함 | API contract, Storage path 정책, DB migration 규칙 |
| 문서 위반이 버그/장애/비용으로 이어짐 | 업로드 정책, 배포 runbook, 환경변수 규칙 |
| 특정 코드베이스에 귀속된 결정 | ADR, 프로젝트 아키텍처 결정, 라이브러리 선택 이유 |
| 자동화/스크립트/CI와 연결됨 | 테스트 방법, 검증 명령, 운영 스크립트 사용법 |
...
구현과 직접 연결되는 계획은 `docs/plans/` 또는 프로젝트 관례를 따른다.
...
- 문서 주제
- 목적: 정책 / ADR / runbook / 아키텍처 / 계획 / 리서치 요약 / 지식화
...
- "repo docs 정본 + Obsidian 요약 둘 다 남길까요?"
...
- repo 정본이어야 하는 문서를 Obsidian에만 남기지 않았는가?
- Obsidian 지식 노트에 프로젝트 실행 정책의 유일한 정본을 두지 않았는가?
