...
**계획 문서 필수 규칙:** Obsidian에 계획 문서를 새로 작성하거나 기존 계획을 크게 수정할 때는 반드시 먼저 `mece-writing` 스킬의 `SKILL.md`를 읽고 적용한다. 계획의 결론, 범위, 실행 단계, 검증, 리스크, 미결정 사항을 동일한 논리 수준으로 분류하고 ME·CE 검증을 거친다.
...
### Step 4: 계획 문서라면 MECE 선행 적용

다음 중 하나에 해당하면 계획 문서로 판단한다.

- 사용자가 계획, 플랜, 구현 계획, 로드맵, 단계별 실행안 작성을 요청한 경우
- 저장 위치가 `Projects/{slug}/agents/plans/`인 경우
- 기존 계획 문서를 구조적으로 개정하는 경우

계획 문서라면 작성 또는 개정 전에 반드시 다음을 수행한다.

1. 설치된 `mece-writing` 스킬의 `SKILL.md`를 읽는다.
2. 계획에 적합한 분류 축을 고른다. 일반적인 구현 계획은 프로세스 기반 커스텀 MECE를 우선 검토한다.
3. 결론을 먼저 제시하고, 대분류를 동일한 논리 수준의 3~5개로 구성한다.
4. 각 대분류의 경계를 명시하고 ME와 CE를 검증한다.
5. 목표, 비목표, 실행 단계, 검증, 리스크, 미결정 사항의 누락과 중복을 확인한다.

프레임워크 선택이 결과를 크게 바꾸는 경우에만 사용자에게 선택을 요청한다. 명백한 구현·운영 계획은 불필요한 인터뷰 없이 프로세스 기반 커스텀 MECE를 적용할 수 있다.

`mece-writing`의 일반 파일명 규칙보다 대상 폴더의 `_index.md` 파일명 규칙이 우선한다.
...
- 계획 문서는 MECE 분류 트리 또는 분류 기준과 최종 ME·CE 검증 근거를 문서에 남긴다.

### Step 6: 새 라우팅 정책이 필요한 경우 — vault 내부 문서를 직접 편집
...
- Obsidian에 글을 작성하는 작업은 일반 코드 변경 완료 검증 대상이 아니므로 `completion-review` 스킬을 실행하지 않는다.
