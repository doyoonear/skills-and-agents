...
**퍼블리싱/디자인/FE 작업 (agent-browser 시각 검증 포함):**
```
┌─────────────┐    ┌──────────┐    ┌────────────────────────┐    ┌──────────────────┐    ┌────────┐
│ Phase 구현  │ → │  테스트   │ → │ agent-browser 시각검증  │ → │ HANDOFF.md 업데이트│ → │  커밋  │
└─────────────┘    └──────────┘    └────────────────────────┘    └──────────────────┘    └────────┘
       ↑                                  │ 문제 발견                                    │
       │                                  ↓                                              │
       │                           ┌──────────┐                                          │
...
1. 컴포넌트 UI 구현
2. 테스트/검증
3. agent-browser 시각 검증 (open → snapshot → screenshot → console 확인)
...
| 우선순위 | 도구 | 용도 |
|----------|------|------|
| **1순위** | agent-browser (스킬) | `open`, `snapshot -i`, `screenshot`, `eval`, `diff snapshot` |
...
```markdown
## Visual Verification
각 Phase 완료 후, 커밋 전에 agent-browser 시각 검증을 실행합니다:
1. agent-browser open <url> && agent-browser wait --load networkidle
2. agent-browser snapshot -i 로 구조 확인
3. agent-browser screenshot 으로 시각적 결과 캡처
4. agent-browser eval 로 콘솔 에러/경고 확인
5. 문제 발견 시 수정 후 재검증
```
