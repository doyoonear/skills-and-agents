#!/bin/bash
# Ralph Loop Stop Hook (통합)
# Claude Code의 종료를 가로채서 반복 실행을 관리합니다.
# planner_type에 따라 standard(ralph-planner) / deep(ralph-deep-planner) 분기 처리.
# exit 0: 종료 허용 | exit 2: 종료 차단 + 프롬프트 재주입

set -euo pipefail

RALPH_DIR=".ralph"
STATE_FILE="$RALPH_DIR/state.md"
PROMPT_FILE="$RALPH_DIR/prompt.md"
PROGRESS_LOG="$RALPH_DIR/progress.log"

if [[ ! -f "$STATE_FILE" ]]; then
  exit 0
fi

# 세션 ID 격리 (다중 세션 충돌 방지)
CURRENT_TERM_SID="${TERM_SESSION_ID:-${ITERM_SESSION_ID:-$$}}"
STATE_TERM_SID=$(grep '^term_session_id:' "$STATE_FILE" 2>/dev/null | sed 's/term_session_id: *//' | tr -d '"' || echo "")

if [[ -n "$STATE_TERM_SID" ]] && [[ "$STATE_TERM_SID" != "null" ]] && [[ -n "$CURRENT_TERM_SID" ]]; then
  if [[ "$STATE_TERM_SID" != "$CURRENT_TERM_SID" ]]; then
    exit 0
  fi
fi

# 상태 읽기
CURRENT_ITER=$(grep '^iteration:' "$STATE_FILE" 2>/dev/null | sed 's/iteration: *//' || echo "0")
MAX_ITER=$(grep '^max_iterations:' "$STATE_FILE" 2>/dev/null | sed 's/max_iterations: *//' || echo "50")
COMPLETION_PROMISE=$(grep '^completion_promise:' "$STATE_FILE" 2>/dev/null | sed 's/completion_promise: *//' | tr -d '"' || echo "")
TASK_SLUG=$(grep '^task_slug:' "$STATE_FILE" 2>/dev/null | sed 's/task_slug: *//' | tr -d '"' || echo "")
PLANNER_TYPE=$(grep '^planner_type:' "$STATE_FILE" 2>/dev/null | sed 's/planner_type: *//' | tr -d '"' || echo "standard")
PLAN_FILE=$(grep '^plan_file:' "$STATE_FILE" 2>/dev/null | sed 's/plan_file: *//' | tr -d '"' || echo "")

# stdin에서 Claude의 마지막 출력 읽기
HOOK_INPUT=$(cat)
CLAUDE_OUTPUT=$(echo "$HOOK_INPUT" | jq -r '.transcript // ""' 2>/dev/null || echo "$HOOK_INPUT")

# 완료 조건 체크
if [[ -n "$COMPLETION_PROMISE" ]] && echo "$CLAUDE_OUTPUT" | grep -q "$COMPLETION_PROMISE"; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] COMPLETED: Found completion promise '$COMPLETION_PROMISE' at iteration $CURRENT_ITER (planner: $PLANNER_TYPE)" >> "$PROGRESS_LOG"

  if [[ "$PLANNER_TYPE" == "deep" ]]; then
    echo "Ralph Deep Loop 완료! (반복 ${CURRENT_ITER}회, 완료 조건: ${COMPLETION_PROMISE})"
    echo "  - /ce:review 자동 실행이 포함된 세션이었습니다."
    if [[ -n "$PLAN_FILE" ]]; then
      echo "  - 계획 파일 보존됨: $PLAN_FILE"
    fi
  else
    echo "Ralph Loop 완료! (반복 ${CURRENT_ITER}회, 완료 조건: ${COMPLETION_PROMISE})"
  fi
  exit 0
fi

# 최대 반복 횟수 체크
if [[ "$CURRENT_ITER" -ge "$MAX_ITER" ]]; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] MAX_ITERATIONS: Reached limit of $MAX_ITER iterations (planner: $PLANNER_TYPE)" >> "$PROGRESS_LOG"

  echo "Ralph Loop 중단: 최대 반복 횟수 도달 (${MAX_ITER}회)"
  echo "완료 조건 '$COMPLETION_PROMISE'이 충족되지 않았습니다."
  exit 0
fi

# 반복 카운터 증가
NEW_ITER=$((CURRENT_ITER + 1))
sed -i.bak "s/^iteration: .*/iteration: $NEW_ITER/" "$STATE_FILE" && rm -f "$STATE_FILE.bak"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] ITERATION $NEW_ITER/$MAX_ITER: Continuing... (planner: $PLANNER_TYPE)" >> "$PROGRESS_LOG"

# 핸드오프 문서 경로
HANDOFF_PATH=""
if [[ -n "$TASK_SLUG" ]]; then
  HANDOFF_PATH="docs/ralph-${TASK_SLUG}/HANDOFF.md"
fi

# 프롬프트 재주입 (exit 2로 종료 차단)
if [[ -f "$PROMPT_FILE" ]]; then
  echo ""
  if [[ "$PLANNER_TYPE" == "deep" ]]; then
    echo "=== Ralph Deep Loop: 반복 $NEW_ITER/$MAX_ITER ==="
  else
    echo "=== Ralph Loop: 반복 $NEW_ITER/$MAX_ITER ==="
  fi
  echo ""

  # 핸드오프 문서를 먼저 읽도록 지시
  if [[ -n "$HANDOFF_PATH" ]] && [[ -f "$HANDOFF_PATH" ]]; then
    echo "먼저 $HANDOFF_PATH 를 읽고 이전 진행 상황을 파악한 후 이어서 작업하세요."
    echo ""
  fi

  # deep 모드: 계획 파일 및 학습 프로토콜 리마인더
  if [[ "$PLANNER_TYPE" == "deep" ]]; then
    if [[ -n "$PLAN_FILE" ]] && [[ -f "$PLAN_FILE" ]]; then
      echo "계획 파일 참조: $PLAN_FILE"
      echo ""
    fi
    echo "[Compound Learning] Phase 완료 시 학습 조건 확인: 예상과 다른 동작, 새 패턴, 긴 디버깅, 미문서화 동작 발견 시 docs/solutions/에 기록하세요."
    echo ""
  fi

  cat "$PROMPT_FILE"
  exit 2
else
  echo "오류: 프롬프트 파일을 찾을 수 없습니다 ($PROMPT_FILE)"
  exit 0
fi
