...
|------|--------------|-------------------|
| 계획 수립 | 자체 프롬프트 생성 | `/ce:plan` 위임 → `docs/plans/` 파일 생성 |
...
**중요**: `/ce:plan`이 실행되면 인터랙티브하게 질문-응답을 거쳐 `docs/plans/` 에 계획 파일을 생성합니다.
...
`/ce:plan`이 생성한 파일 경로를 확보합니다:
- 예: `docs/plans/2026-03-09-feat-payment-migration-plan.md`
...
## Plan Reference
이 작업의 상세 계획은 다음 파일을 참조: `docs/plans/[plan-file].md`
...
## Implementation Plan
`docs/plans/[plan-file].md`의 Implementation Plan을 따릅니다.
각 Phase 완료 시:
1. 테스트/검증
...
`docs/plans/` 파일은 그대로 유지되므로 계획도 참조 가능.
...
| **계획 파일 보존** | `docs/plans/` 파일은 루프 완료 후에도 유지 |
